...
**WE NEED SOMEONE TO WRITE ABOUT HOW WE CAN ONLY USE NUMERIC VARIABLE IN CLUSTERING ANALYSIS**

**DID WE NOT INCLUDE ANY VARIABLES IN OUR FINAL DATASET? WHY DID WE EXCLUDE THESE FROM OUR ANALYSIS?**
...
ggplot(aes(x = danceability, y = energy), data = Data) + 
  geom_point(col = "springgreen2") +
  labs(title = "Scatterplot of Spotify 2018 Top Hits by Danceability and Energy",
       x = "Danceability",
       y = "Energy") +
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
...
**WRITE A BIT ABOUT THESE VISUALIZATIONS**
...
```{r, echo = F, fig.cap = "Dendrograms for the Single and Average Linking Methods for Hierarchical Clustering."}
Dist <- dist(NumericData)

par(mfrow = c(1, 2))
plot(hclust(Dist, method = 'single'), main = "Cluster Dendrogram: Single Linkage")
plot(hclust(Dist, method = 'average'), main = "Cluster Dendrogram: Average Linkage")
```
...
The model based clustering method assumes distributions for both the population and the subpopulation. While the pdfs are defined for these distributions, they will not be discussed in detail in this paper. 
...
rownames(tab) <- c('Hierarchical Clustering','K-Means Clustering','Model-Based Clustering')
